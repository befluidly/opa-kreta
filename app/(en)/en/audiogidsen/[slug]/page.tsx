...
export async function generateStaticParams() {
  const posts = getAllPosts("audiogidsen", "en");
  return posts.map((p) => ({ slug: p.canonicalSlug.replace("audiogidsen/", "") }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const post = getPostByCanonicalSlug(`audiogidsen/${slug}`, "en");
  if (!post) return {};
...
export default async function EnglishAudiogidsPage({ params }: PageProps) {
  setRequestLocale("en");
  const { slug } = await params;
  const post = getPostByCanonicalSlug(`audiogidsen/${slug}`, "en");
  if (!post) {
  ...
  return (
    <Layout articleAlternates={alternates}>
      ...
      <div className="max-w-screen-md mx-auto px-4 mt-8">
        <BackButton fallbackHref="/en" label="← Back" />
        ...
        {stops.length === 0 ? (
          <p className="text-gray-600 font-body italic mb-20">
            This audio guide is still in preparation.
          </p>
        ) : (
